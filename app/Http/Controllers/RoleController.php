...
class RoleController extends Controller
{
    ...
    public function giveRoleToUser(Request $request, $roleId, $userId)
    {
        ...
        $doesRoleExist = UserHasRole::where('role_id', $roleId)->where('user_id', $userId)->exists();
        if($doesRoleExist) {
            return response()->json(['error' => 'Role already assigned']);
        }
    ...
    /************************************* ADMIN ROLES TABLE (CRUD operations) *********************************************/
    public function getRoleDetails($id)
    {
        $user = Role::where('id', $id)->get()->first();
        return response()->json(['role' => $user], 200);
    }

    public function givePermissionToRole(Request $request, $roleId, $userId)
    {
//        echo $roleId  .' '. $userId;
        $doesRoleExist = RoleHasPermission::where('permission_id', $roleId)->where('role_id', $userId)->exists();
//        echo $doesRoleExist;
//        return;

        if($doesRoleExist) {
            return response()->json(['error' => 'Permission already assigned']);
        }
    ...
    static function getPermissionsNames()
    {
        $roles = Permission::latest()->get();

        return response()->json($roles);
    }

    public function deletePermissionFromRole(Request $request, $role_id, $user_id) {
        $row = RoleHasPermission::where('permission_id', $role_id)
    ...
    public function getRolePermissionsNames($id) {
        $user = Role::find($id);
        if ($user) {
            $roles = $user->permissions()->get();
            return response()->json($roles);
        }
        return response()->json(['failed'=> 'permission id does not exist']);
    }
