...
function getUserRolesNames(userId, event) {
    fetch('/users/getUserRolesNames/' + userId, {
        method: 'GET',
        headers: {
            'X-CSRF-TOKEN': csrfToken
        }
        ...
        .then(data => {
            ...
            if(userId === undefined) {
                ...
                checkboxesContainer.textContent = 'There is only USER role'
                return;
            }
...
function unsetRoles(userId, unsetBtn) {
    ...
    function unset() {
        ...
        rolesIDs.forEach(roleId => {
            fetch('/role/deleteRoleFromUser/' + roleId + '/'+ userId, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': csrfToken
                }
...
function assignRoles(userId, assignBtn) {
    ...
    function assign() {
        console.log(rolesIDs);
        rolesIDs.forEach(roleId => {
            fetch('/role/giveRoleToUser/' + roleId + '/'+ userId, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken
                }
