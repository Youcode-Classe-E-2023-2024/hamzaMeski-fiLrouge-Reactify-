    ...
    <tr class="user-table-row">
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div>
                    <div class="text-sm leading-5 text-gray-300">#${user.id}</div>
                </div>
            </div>
        </td>
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="userName text-sm leading-5 text-blue-500">${user.name}</div>
        </td>
...
function getUserRolesNames(userId, event) {
    fetch('/role/getRolePermissionsNames/' + userId, {
        method: 'GET',
        headers: {
            'X-CSRF-TOKEN': csrfToken
        }
        ...
        .then(data => {
            // console.log(data);
            // return;
            const userId = data[0]?.pivot?.role_id;
            ...
            if(userId === undefined) {
                ...
                checkboxesContainer.textContent = 'This role have no permission yet'
                return;
            }
...
function unsetRoles(userId, unsetBtn) {
    ...
    function unset() {
        ...
        rolesIDs.forEach(roleId => {
            fetch('/permission/deletePermissionFromRole/' + roleId + '/'+ userId, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': csrfToken
                }
...
function getRolesNames(userId, event) {
    fetch('permission/getPermissionsNames', {
        method: 'GET',
        headers: {
            'X-CSRF-TOKEN': csrfToken
        }
...
function assignRoles(userId, assignBtn) {
    ...
    function assign() {
        console.log(rolesIDs);
        rolesIDs.forEach(roleId => {
            fetch('/permission/givePermissionToRole/' + roleId + '/'+ userId, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken
                }
...
function getUserInfo(userId, event) {
    fetch('/role/getRoleDetails/' + userId, {
        method: 'GET',
        headers: {
            'X-CSRF-TOKEN': csrfToken
        }
...
function searchUserName() {
    function searchUserName() {
        ...
        const userTableRow = document.querySelectorAll('.user-table-row');

        for(let i = 0; i < elements.length; i++){
            const userName = elements[i].textContent.trim().toLowerCase();
            if (userName.includes(query)) {
                userTableRow[i].style.display = 'block';
            } else {
